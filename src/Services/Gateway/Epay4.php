...
class Epay4 extends AbstractPayment
{
    ...
    {
        $this->epay['apiurl'] = $_ENV['epay4']['epay_api_url'];//
        $this->epay['partner'] = $_ENV['epay4']['epay_pid'];//易支付商户pid
        $this->epay['key'] = $_ENV['epay4']['epay_key'];//易支付商户Key
        $this->epay['sign_type'] = strtoupper('MD5'); //签名方式
        $this->epay['input_charset'] = strtolower('utf-8');//字符编码
        $this->epay['transport'] = $_ENV['epay4']['transport'];//协议 http 或者https
    }
    ...
    public function MetronPay($type, $price, $buyshop, $paylist_id=0)
    {
        ...
        $data = array(
            ...
            "notify_url" => $_ENV['baseUrl'] . "/payment/notify/epay4",
